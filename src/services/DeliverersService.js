// services/DeliverersService.ts
'use client'

import ServicesHelper from "@/src/utils/helpers/ServicesHelper";

const TABLE_NAME = 'deliverers'

class DeliverersService {
	...
	static async count(jwt) {
		try {
			...
			console.error('Error in get of DeliverersService:', error);
			throw error;
		}
	}
	...
	static async createOne(jwt, label, description = '') {
		try {
			...
			console.error('Error in createOne of DeliverersService:', error);
			throw error;
		}
	}
	...
	static async deleteOne(jwt, id) {
		try {
			...
			console.error('Error in deleteOne of DeliverersService:', error);
			throw error;
		}
	}
	 ...
	 */
		static async updateOne(jwt, id, modifWanted) {
			try {
				...
				console.error('Error in updateOne of DeliverersService:', error);
				throw error;
			}
		}
}
